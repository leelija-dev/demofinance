...
class LoanApplicationView(AgentSessionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for app in qs:
            ...
            customer = getattr(app, 'customer', None)
            customer_detail_snapshot = app.customer_snapshot.get('customer_details', {}) if app.customer_snapshot else {}
            is_old_loan = customer.loan_application_id != app.loan_ref_no
        ...
        for attempt in range(max_retries):
            try:
                with transaction.atomic():
                    ...
                    # Populate customer snapshot after all related data is created
                    loan_application.populate_customer_snapshot()
                    ...
                    try:
                        ...
                        # Branch email
                        # Prefer the saved loan_application.branch, but fall back to the
                        # branch resolved earlier in this request/session.
                        branch_obj = (
                            getattr(loan_application, 'branch', None)
                            or branch
                            or getattr(customer, 'branch', None)
                            or getattr(agent, 'branch', None)
                            or getattr(created_by_branch_manager, 'branch', None)
                        )
...
class LoanCategoryListAPI(APIView):
    from demo.loan.filters import filter_by_parent_hq
    @filter_by_parent_hq(LoanCategory, return_empty_on_fail=True)
    def get(self, request):
        ...
        if shop_status:
            if shop_status == 'inactive':
                ...
                categories = categories.filter(main_category__is_shop_active=True)
...
class LoanSubCategoryTenureListAPI(APIView):
    from demo.loan.filters import filter_by_parent_hq
    @filter_by_parent_hq(LoanTenure)
    def get(self, request):
...
class LoanMainCategoryListAPI(APIView):
    ...
    def get(self, request):
        ...
        base_queryset = LoanMainCategory.objects.filter()
...
class LoanDeductionsListAPI(APIView):
    from demo.loan.filters import filter_by_parent_hq
    @filter_by_parent_hq(Deductions)
    def get(self, request):
        main_category_id = request.GET.get('main_category_id')
        if not main_category_id:
            return Response({"error": "main_category_id is required"}, status=400)
