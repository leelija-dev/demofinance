...
from loan.serializers import (
    ...
    LoanDisbursedListSerializer,
    LoanEMIScheduleSerializer,
...
def branch_home(request):
    ...
    if logged_user_id:
        try:
            logged_user = BranchEmployee.objects.get(id=logged_user_id, is_active=True)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not getattr(logged_user.branch, "status", True):
                ...
                return Response(
                    {"detail": "Branch is inactive. Please contact headquarters."},
...
class BranchDashboardView(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            branch_employee = BranchEmployee.objects.get(
                id=logged_user_id, is_active=True
            )
        except BranchEmployee.DoesNotExist:
            request.session.flush()
            login_url = reverse("branch:branch_login_page")
    ...
    def get_context_data(self, **kwargs):
        ...
        if logged_user_id:
            ...
            if branch:
                context["branch_approve_count"] = LoanApplication.objects.filter(
                    branch=branch, status__in=["branch_approved", "hq_approved"]
                ).count()
                context["branch_pending_count"] = LoanApplication.objects.filter(
...
def profile_image_update(request):
    ...
    if not photo_file:
        return JsonResponse(
            {"success": False, "message": "No photo provided."}, status=400
        )
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "message": "Agent not found."}, status=404
        )
...
def change_password(request):
    if request.method == "POST":
        ...
        if not logged_user_id:
            return JsonResponse(
                {"success": False, "message": "Not logged in."}, status=401
            )
...
class NewLaonApplication(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        # Initialize context
        context = {"is_active": True, "error_message": None}
...
class LoanApplicationEdit(APIView):
    def post(self, request, customer_id, *args, **kwargs):
        ...
        if updated:
            customer.save()
            return Response(
                {"success": True, "message": "Customer information updated."},
                status=status.HTTP_200_OK,
            )
        else:
            return Response(
...
@method_decorator(
    branch_permission_required("view_loans", "approve_loan", "reject_loan"),
    name="dispatch",
)
    ...
    def get(self, request, *args, **kwargs):
        ...
        requests = (
            DocumentRequest.objects.filter(
                branch=branch, loan_application__isnull=False
            )
...
class CompletedLoansAPI(APIView):
    def get(self, request, *args, **kwargs):
        ...
        close_qs = (
            LoanCloseRequest.objects.filter(branch=branch, status="approved")
            .select_related(
                "loan_application",
            ...
            results.append(
                {
                    ...
                    "loan_amount": (
                        float(detail.loan_amount)
                        if detail and detail.loan_amount is not None
                        else None
                    ),
        ...
        try:
            ...
            try:
                customer = CustomerDetail.objects.get(
                    customer_id=data["customer_id"], branch=branch
                )
                ...
                return Response(
                    {"detail": "Customer or document request not found."},
                    status=status.HTTP_404_NOT_FOUND,
                )
            ...
            if customer_document and field_name:
                setattr(
                    customer_document,
                    field_name,
                    getattr(reupload, "uploaded_file", None),
                )
            ...
            remaining_requests = DocumentRequest.objects.filter(
                loan_application=loan_application, is_resolved=False
            ).exists()
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            try:
                ...
                return Response(
                    {"detail": "Loan application not found for this customer."},
                    status=status.HTTP_404_NOT_FOUND,
                )
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            try:
                ...
                return Response(
                    {"detail": "Document reupload not found."},
                    status=status.HTTP_404_NOT_FOUND,
                )
            ...
            if data["decision"] == "approved":
                ...
                customer_document = getattr(
                    reupload.loan_application, "documents", None
                )
                field_map = {
                    "id_proof": "id_proof",
                    "income_proof": "income_proof",
                ...
                all_reuploads = DocumentReupload.objects.filter(
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if loan_app.status in ["document_requested", "document_requested_by_hq"]:
                return Response(
                    {"detail": "Cannot approve while documents are requested."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            if DocumentRequest.objects.filter(
                loan_application=loan_app, is_resolved=False
            ).exists():
                return Response(
                    {"detail": "Cannot approve while document requests are pending."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            loan_app.save()
            return Response(
                {"success": True, "new_status": loan_app.status},
                status=status.HTTP_200_OK,
            )
        except BranchEmployee.DoesNotExist:
            return Response(
...
class RejectApplicationAPIView(APIView):
    def post(self, request, *args, **kwargs):
        try:
            ...
            if loan_app.status in ["document_requested", "document_requested_by_hq"]:
                return Response(
                    {"detail": "Cannot reject while documents are requested."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            if DocumentRequest.objects.filter(
                loan_application=loan_app, is_resolved=False
                ...
                return Response(
                    {"detail": "Cannot reject while document requests are pending."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class BranchApplicationDetailAPI(APIView):
    def get(self, request, customer_id, loan_ref_no):
        ...
        try:
            ...
            loan_app = (
                LoanApplication.objects.filter(
                    loan_ref_no=loan_ref_no, customer=customer
                )
                .order_by("-submitted_at")
                .first()
            )
            ...
            loans = loan_app.loan_details.select_related(
                "loan_category__main_category"
            ).all()
            ...
            is_shop_active = bool(
                ...
                and bool(
                    getattr(
                        first_loan.loan_category.main_category, "is_shop_active", False
                    )
                )
            )
            ...
            def get_best_document(loan_application, doc_type, original_file):
                if doc_type in ["residential_proof", "residential_proof_file"]:
                    reuploads = DocumentReupload.objects.filter(
                        loan_application=loan_application,
                        document_type__in=[
                            "residential_proof",
                    ...
                    reuploads = DocumentReupload.objects.filter(
                        loan_application=loan_application, document_type=doc_type
                    ).order_by("-uploaded_at")
                latest = reuploads.first()
                if latest and latest.uploaded_file:
                    approved_review = latest.reviews.filter(
                        decision="approved"
                    ).exists()
                    if approved_review:
                        return latest.uploaded_file.url
            ...
            if documents:
                documents_data = {
                    "id_proof": get_best_document(
                        loan_app, "id_proof", documents.id_proof
                    ),
                    "pan_card_document": get_best_document(
                        loan_app,
                        "pan_card_document",
                        getattr(documents, "pan_card_document", None),
                    ),
                    "id_proof_back": get_best_document(
                        loan_app, "id_proof_back", documents.id_proof_back
                    ),
                    "income_proof": get_best_document(
                        loan_app, "income_proof", documents.income_proof
                    ),
                    "photo": get_best_document(loan_app, "photo", documents.photo),
                    "signature": get_best_document(
                        loan_app, "signature", documents.signature
                    ),
                    "guarantor_id_proof": get_best_document(
                        loan_app, "guarantor_id_proof", documents.guarantor_id_proof
                    ),
                    "collateral": get_best_document(
                        loan_app, "collateral", documents.collateral
                    ),
                    "residential_proof_file": get_best_document(
                        loan_app, "residential_proof", documents.residential_proof_file
                    ),
                }

            customer_detail_snapshot = (
                loan_app.customer_snapshot.get("customer_details", {})
                if loan_app.customer_snapshot
                else {}
            )
            customer_address_snapshot = (
                loan_app.customer_snapshot.get("address", {})
                if loan_app.customer_snapshot
                else {}
            )
            customer_documents_snapshot = (
                loan_app.customer_snapshot.get("documents", {})
                if loan_app.customer_snapshot
                else {}
            )
            customer_bank_details_snapshot = (
                loan_app.customer_snapshot.get("bank_details", {})
                if loan_app.customer_snapshot
                else {}
            )
            ...
            data = {
                ...
                "full_name": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "full_name"
                ),
                "father_name": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "father_name"
                ),
                "guarantor_name": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "guarantor_name"
                ),
                "date_of_birth": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "date_of_birth"
                ),
                "gender": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "gender"
                ),
                "contact": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "contact"
                ),
                "email": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "email"
                ),
                "adhar_number": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "adhar_number"
                ),
                "pan_number": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "pan_number"
                ),
                "voter_number": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "voter_number"
                ),
                "status": loan_app.status,
                "submitted_at": loan_app.submitted_at,
                "customer_submitted_at": self._get_customer_details(
                    customer, is_old_loan, customer_detail_snapshot, "submitted_at"
                ),
                ...
                "shop": (
                    {"shop_id": loan_app.shop.shop_id, "name": loan_app.shop.name}
                    if loan_app.shop
                    else None
                ),
                ...
                "address": (
                    customer_address_snapshot
                    if is_old_loan
                    else CustomerAddressSerializer(address).data if address else None
                ),
                "customer_account": (
                    customer_bank_details_snapshot
                    if is_old_loan
                        ...
                        if hasattr(customer, "account") and customer.account
                        else None
                    )
                ),
                ...
                "document_reuploads": [
                    {
                        ...
                        "document_type": reupload.document_type,
                        "uploaded_file": (
                            reupload.uploaded_file.url
                            if reupload.uploaded_file
                            else None
                        ),
                        "agent_note": reupload.agent_note,
                        "uploaded_by": (
                            reupload.uploaded_by.full_name
                            if reupload.uploaded_by
                            else None
                        ),
                        ...
                        "reviews": [
                            {
                                ...
                                "reviewed_by": (
                                    review.reviewed_by.user.get_full_name()
                                    if (
                                        review.reviewed_by
                                        and hasattr(review.reviewed_by, "user")
                                        and hasattr(
                                            review.reviewed_by.user, "get_full_name"
                                        )
                                    )
                ...
                "document_reviews": [
                    {
                        ...
                        "reviewed_by": (
                            ...
                            if (
                                review.reviewed_by
                                and hasattr(review.reviewed_by, "user")
                                and hasattr(review.reviewed_by.user, "get_full_name")
                            )
                            else (
                                str(review.reviewed_by) if review.reviewed_by else None
                            )
                        ),
                        "reviewed_at": review.reviewed_at,
                        "document_type": (
                            review.document_reupload.document_type
                            if review.document_reupload
                            else None
                        ),
                    }
                ...
                "document_requests": [
                    {
                        ...
                        "requested_by": (
                            ...
                            and hasattr(request.requested_by, "get_full_name")
                            else (
                                f"{request.requested_by.first_name} {request.requested_by.last_name}"
                                if request.requested_by
                                and hasattr(request.requested_by, "first_name")
                                else (
                        ...
                        "requested_by_hq": (
                            getattr(
                                request.requested_by_hq,
                                "get_full_name",
                                lambda: (
                                    str(request.requested_by_hq)
                                    if request.requested_by_hq
                                    else None
                                ),
                            )()
                            if request.requested_by_hq
...
class BranchApplicationRejectedViewByHQAPI(APIView):
    def get(self, request):
        ...
        try:
            ...
            applications = LoanApplication.objects.filter(
                branch=branch, status__in=["rejected_by_branch", "hq_rejected"]
            ).select_related("customer", "agent")
            serializer = LoanApplicationListSerializer(applications, many=True)
...
class BranchApplicationApprovedViewByHQAPI(APIView):
    def get(self, request):
        ...
        try:
            ...
            branch = branch_manager.branch
            applications = LoanApplication.objects.filter(
                branch=branch, status="hq_approved"
            ).select_related("customer", "agent")
            serializer = LoanApplicationListSerializer(applications, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except BranchEmployee.DoesNotExist:
            return Response(
                {"details": "Branch manager not found"},
                status=status.HTTP_404_NOT_FOUND,
            )
        except Exception as e:
            return Response(
...
def branch_manager_info_api(request):
    ...
    if logged_user_id:
        try:
            manager = BranchEmployee.objects.select_related("branch").get(
                id=logged_user_id
            )
            ...
            if manager.is_manager:
                ...
                manager_raw = (
                    BranchEmployee.objects.filter(
                        is_manager=True, branch_id=manager.branch_id
                    )
            ...
            if branch:
                branch_data = {
                    ...
                    "wallet_balance": (
                        str(branch.wallet_balance)
                        if hasattr(branch, "wallet_balance")
                        else None
                    ),
            ...
            return JsonResponse(
                {
                    ...
                    "manager": {
                        ...
                        "date_of_birth": (
                            manager.date_of_birth.strftime("%d-%m-%Y")
                            if manager.date_of_birth
                            else None
                        ),
                        ...
                        "updated_at": (
                            manager.updated_at.strftime("%d-%m-%Y")
...
########### disbursement by HQ ############
@method_decorator(
    branch_permission_required("view_loans", "approve_loan", "disburse_loan"),
    name="dispatch",
)
...
class DisbursedByHQAPI(APIView):
    def get(self, request, *args, **kwargs):
        ...
        applications = (
            LoanApplication.objects.filter(branch=branch, status__in=pending_statuses)
            .select_related("customer", "agent")
...
class DisbursedDetailByHQAPI(APIView):
    def get(self, request, loan_ref_no, *args, **kwargs):
        ...
        pending_statuses = ["disbursed"]
        applications = LoanApplication.objects.filter(
            branch=branch, status__in=pending_statuses, loan_ref_no=loan_ref_no
        ...
        try:
            ...
            try:
                ...
                if disbursement_date > timezone.now().date():
                    return Response(
                        {"detail": "Disbursement date cannot be in the future."},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            except ValueError:
                return Response(
                    {"detail": "Invalid disbursement date format."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            try:
                ...
                return Response(
                    {"detail": "Invalid source account selected."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if loan_application.status == "disbursed_fund_released":
                return Response(
                    {"detail": "Loan is already disbursed."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            with transaction.atomic():
                ...
                destination_name = (
                    loan_application.customer.full_name
                    if loan_application.customer
                    else None
                )
                ...
                if loan_application.shop:
                    ...
                    shop_has_accounts = ShopBankAccount.objects.filter(
                    ...
                    if not shop_has_accounts:
                        return Response(
                            {
                                "detail": "Please add bank account for this shop before disbursement."
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
                    ...
                    if not shop_bank_account_id:
                        return Response(
                            {"detail": "Please select shop bank account."},
                            status=status.HTTP_400_BAD_REQUEST,
                        )
                    ...
                    try:
                        shop_bank_account = (
                            ShopBankAccount.objects.select_for_update().get(
                        ...
                        return Response(
                            {"detail": "Invalid shop bank account selected."},
                            status=status.HTTP_400_BAD_REQUEST,
                        )
                ...
                first_emi = (
                    LoanEMISchedule.objects.filter(loan_application=loan_application)
                ...
                coa_code = (
                    "120" if freq == "daily" else ("121" if freq == "weekly" else "120")
                )
                coa = ChartOfAccount.objects.filter(code=coa_code).first()
                coa_head = coa.head_of_account if coa else "Loan disbursement"
                coa_desc = (
                    coa.description
                    if coa and coa.description
                    else f"Loan disbursement for {freq or 'daily'} collection"
                )
                ...
                if shop_bank_account:
                    shop_bank_account.current_balance = (
                        shop_bank_account.current_balance + net_amount
                    ).quantize(Decimal("0.01"))
                    shop_bank_account.save(
            ...
            loan_period = LoanPeriod.objects.filter(
                loan_application=loan_application
            ).first()
            if not loan_period:
                raise Exception(
                    "Loan period details not found for this loan application"
                )
            ...
            logger.info(
                f"Processing EMI schedule for loan application: {loan_application.loan_ref_no}"
            )
            ...
            try:
                ...
                raise Exception(
                    "dateutil.relativedelta is required for EMI schedule calculation"
                )
            ...
            with transaction.atomic():
                ...
                for i in range(1, num_installments + 1):
                    ...
                    principal_portion = (principal / num_installments).quantize(
                        Decimal("0.01"), rounding=ROUND_HALF_UP
                    )
                    interest_portion = (total_interest / num_installments).quantize(
                        Decimal("0.01"), rounding=ROUND_HALF_UP
                    )
                    if i == num_installments:
                        principal_portion = principal - (
                            principal_portion * (num_installments - 1)
                        )
                        interest_portion = total_interest - (
                            interest_portion * (num_installments - 1)
                        )
                        principal_portion = principal_portion.quantize(
                            Decimal("0.01"), rounding=ROUND_HALF_UP
                        )
                        interest_portion = interest_portion.quantize(
                            Decimal("0.01"), rounding=ROUND_HALF_UP
                        )
                    emi_schedule = LoanEMISchedule.objects.create(
                        ...
                        frequency=frequency,
                        installment_amount=emi.quantize(
                            Decimal("0.01"), rounding=ROUND_HALF_UP
                        ),
                ...
                if loan_application.created_by_agent:
                    ...
                    for emi in LoanEMISchedule.objects.filter(
                        loan_application=loan_application
                    ):
                        if not EmiAgentAssign.objects.filter(
                            emi=emi, is_active=True
                        ).exists():
                            EmiAgentAssign.objects.create(
            ...
            try:
                ...
                print(
                    f"HQ_NOTIFICATION_EMAIL: {getattr(settings, 'HQ_NOTIFICATION_EMAIL', 'Not set')}\n"
                )
                ...
                # 1. Customer email
                try:
                    if hasattr(loan_application, "customer") and getattr(
                        loan_application.customer, "email", None
                    ):
                ...
                try:
                    ...
                    if period and hasattr(period, "first") and period.first():
                        ...
                        if (
                            loan_detail
                            and hasattr(loan_detail, "first")
                            and loan_detail.first()
                            and getattr(loan_detail.first(), "interest_rate", None)
                        ):
                            actual_percentage = (
                                loan_detail.first().interest_rate.rate_of_interest
                            )
                        else:
                ...
                loan_detail = (
                    loan_application.loan_details.first()
                    if loan_application.loan_details.exists()
                    else None
                )
                ...
                context = {
                    ...
                    "customer_name": (
                        loan_application.customer.full_name
                        if hasattr(loan_application, "customer")
                        and loan_application.customer
                        else "N/A"
                    ),
                    "customer_contact": (
                        loan_application.customer.contact
                        if hasattr(loan_application, "customer")
                        and loan_application.customer
                        else "N/A"
                    ),
                    ...
                    "emi": (
                        loan_detail.emi
                        if loan_detail and hasattr(loan_detail, "emi")
                        else "N/A"
                    ),
                    "tenure": (
                        loan_detail.tenure
                        if loan_detail and hasattr(loan_detail, "tenure")
                        else "N/A"
                    ),
                    ...
                    "tax_charges": (
                        str(tax_charges) if tax_charges is not None else "N/A"
                    ),
                ...
                try:
                    message_html = render_to_string(
                        "loan/loan_application_email.html", context
                    )
                    print("[Email] Successfully rendered HTML template")
                except Exception as template_error:
                    message_html = None
                    print(
                        f"[Email Error] Error rendering HTML template: {str(template_error)}"
                    )
                ...
                try:
                    if hasattr(loan_application, "customer") and getattr(
                        ...
                        if customer_email:
                            print(
                                "[Email] Generating PDF for customer email attachment..."
                            )
                            ...
                            html_content = render_to_string(
                                "loan-disbursed/loan_disbursed_fund_pdf.html",
                                pdf_context,
                            )
                            # Generate PDF using the same method as GenerateLoanPDFAPI
                            pdf_content = self._generate_pdf_for_email(html_content)
                            print(
                                "[Email] PDF generated successfully for email attachment"
                            )
                except Exception as pdf_error:
                    print(
                        f"[Email Error] Failed to generate PDF for email: {str(pdf_error)}"
                    )
                    pdf_content = None
                ...
                for recipient in recipient_list:
                    try:
                        ...
                        if recipient == customer_email and pdf_content:
                            filename = (
                                f"loan_disbursement_{loan_application.loan_ref_no}.pdf"
                            )
                        ...
                        print(
                            f"[Email Error] Failed to send email to {recipient}: {str(send_error)}"
                        )
                        import traceback

                        print(traceback.format_exc())
                print(
                    f"[Email] Email send complete. Success: {success_count}/{len(recipient_list)}"
                )
                ...
                for attempt in (1, 2):
                    try:
                        ...
                        if attempt == 1 and (
                            ...
                            or "was just installed or updated" in msg
                        ):
...
@method_decorator(
    branch_permission_required("view_loans", "disburse_loan"), name="dispatch"
)
class DisbursedFundRelease(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        applications_qs = (
            LoanApplication.objects.filter(
                branch=branch, status="disbursed_fund_released"
            )
            .select_related("customer", "agent")
        ...
        if disbursed_from:
            applications_qs = applications_qs.filter(
                disbursed_at__date__gte=disbursed_from
            )
        if disbursed_to:
            applications_qs = applications_qs.filter(
                disbursed_at__date__lte=disbursed_to
            )
        ...
        try:
            page_links = list(
                paginator.get_elided_page_range(
                    disbursed_page.number, on_each_side=2, on_ends=1
                )
            )
        except Exception:
...
########################################################################
@method_decorator(
    branch_permission_required("view_fund", "add_fund_record"), name="dispatch"
)
class WalletView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["can_transect"] = (
            branch_employee.is_manager or branch_employee.has_perm("add_fund_record")
        )
        ...
        tx_qs = (
            BranchTransaction.objects.filter(branch_id=branch_id)
            .select_related("disbursement_log", "branch_account")
        ...
        context["transactions_without_disbursement"] = tx_qs.filter(
            disbursement_log__isnull=True
        )
        ...
        if date_from:
            fund_transfers_in = fund_transfers_in.filter(
                transfer_date__date__gte=date_from
            )
        if date_to:
            fund_transfers_in = fund_transfers_in.filter(
                transfer_date__date__lte=date_to
            )
        ...
        if filter_mode:
            fund_transfers_in = fund_transfers_in.filter(
                payment_mode__iexact=filter_mode
            )
        if filter_code:
            fund_transfers_in = fund_transfers_in.filter(
                Q(hq_transaction__code=filter_code)
                | Q(branch_transaction__code=filter_code)
            )
        if filter_purpose:
            ...
            all_combined_transactions.append(
                {
                    ...
                    "code": (
                        ft.hq_transaction.code
                        if ft.hq_transaction and ft.hq_transaction.code
                        else ""
                    ),
                    "payment_mode": ft.payment_mode if ft.payment_mode else "",
                    "description": (
                        ft.hq_transaction.description
                        if ft.hq_transaction
                        else (
                            ft.branch_transaction.description
                            if ft.branch_transaction
                            else ""
                        )
                    ),
                    "id": ft.transfer_id,
        ...
        context["disbursement_logs"] = (
            DisbursementLog.objects.filter(disbursed_by_id=branch_id)
            .select_related(
        ...
        monthly_disbursements = DisbursementLog.objects.filter(
            disbursed_by_id=branch_id, created_at__gte=first_day_of_month
        )
        ...
        context["chart_of_accounts"] = ChartOfAccount.objects.all().order_by(
            "main_type", "sl_no", "head_of_account"
        )
        context["hq_wallets"] = HeadquartersWallet.objects.all().order_by(
            "type", "bank_name", "account_number"
        )
        context["accounts"] = BranchAccount.objects.filter(
            branch_id=branch_id, type="BANK"
        ).order_by("account_number", "bank_name")
        context["branch_employees"] = BranchEmployee.objects.filter(
            branch_id=branch_id
        ).order_by("employee_id")
        context["total_cash"] = BranchAccount.objects.filter(
            branch_id=branch_id, type="CASH"
        ).aggregate(total_cash=Sum("current_balance"))["total_cash"] or Decimal("0.00")
        context["total_bank"] = BranchAccount.objects.filter(
            branch_id=branch_id, type="BANK"
        ).aggregate(total_bank=Sum("current_balance"))["total_bank"] or Decimal("0.00")
...
class MoneyTransferToHQAPI(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not payment_mode:
                return Response(
                    {"detail": "Payment mode is required."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if payment_mode == "bank":
                if not bank_payment_method or not account_number or not account_id:
                    return Response(
                        {"detail": "Bank payment method is required."},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            ...
            try:
                ...
                return Response(
                    {"detail": "Invalid purpose selected."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            with transaction.atomic():
                ...
                if payment_mode == "cash":
                    ...
                    try:
                        ...
                        cash_account.current_balance = (
                            Decimal(cash_account.current_balance) - amount
                        ).quantize(Decimal("0.01"))
                        cash_account.save(
                            update_fields=["current_balance", "updated_at"]
                        )
                        ...
                        return Response(
                            {"detail": "Cash account not found for this branch."},
                            status=status.HTTP_400_BAD_REQUEST,
                        )
                    ...
                    if not bank_payment_method or not account_number or not account_id:
                        return Response(
                            {
                                "detail": "Bank payment method, account number, and account ID are required."
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
                    ...
                    try:
                        bank_account = BranchAccount.objects.get(
                            id=account_id,
                            branch=branch,
                            type="BANK",
                            account_number=account_number,
                        ...
                        bank_account.current_balance = (
                            Decimal(bank_account.current_balance) - amount
                        ).quantize(Decimal("0.01"))
                        bank_account.save(
                            update_fields=["current_balance", "updated_at"]
                        )
                        ...
                        return Response(
                            {
                                "detail": "Bank account not found or does not belong to this branch."
                            },
                            status=status.HTTP_404_NOT_FOUND,
                        )
                else:
                ...
                bt = BranchTransaction.objects.create(
                    branch=branch,
                    branch_account=(
                        selected_account if "selected_account" in locals() else None
                    ),
                    ...
                    amount=amount,
                    description=f"{purpose_account.head_of_account} - "
                    + (note or "Transfer to Headquarters"),
                ...
                hq_tx = HeadquartersTransactions.objects.create(
                    ...
                    transaction_type="credit",
                    amount=amount,
                    description=f"Transfer from {branch.branch_name}: {purpose_account.head_of_account} - "
                    + (note or ""),
                    purpose=purpose_account.head_of_account,
                    code=purpose_account.code,
                ...
                hq_wallet.balance = (Decimal(hq_wallet.balance) + amount).quantize(
                    Decimal("0.01")
                )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if transfer_to_from is None or str(transfer_to_from).strip() == "":
                return Response(
                    {"detail": "Transfer to/from is required."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if payment_mode.lower() == "bank" and not bank_payment_method:
                return Response(
                    {
                        "detail": "Bank payment method is required for bank transactions."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            with transaction.atomic():
                ...
                if payment_mode.lower() == "cash":
                    ...
                    if (
                        transaction_type == "debit"
                        and cash_account.current_balance < amount
                    ):
                        return Response(
                            {
                                "detail": f"Insufficient cash balance. Available: ₹{cash_account.current_balance:.2f}"
                    ...
                    cash_account.save(
                        update_fields=["current_balance", "updated_at", "updated_by"]
                    )

                elif payment_mode.lower() == "bank":
                    if not account_id:
                        return Response(
                            {"detail": "Account ID required for bank transaction."},
                            status=status.HTTP_400_BAD_REQUEST,
                        )

                    try:
                        bank_account = BranchAccount.objects.get(
                            id=account_id, type="BANK", branch=branch
                        )
                    except BranchAccount.DoesNotExist:
                        return Response(
                    ...
                    if (
                        transaction_type == "debit"
                        and bank_account.current_balance < amount
                    ):
                        return Response(
                            {
                                "detail": f"Insufficient bank balance in {bank_account.bank_name} ({bank_account.account_number}). Available: ₹{bank_account.current_balance:.2f}"
...
class GenerateLoanPDFAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if not loan_ref_no:
                return Response(
                    {"error": "Loan reference number is required"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            try:
                logo_path = os.path.join(
                    settings.BASE_DIR, "static", settings.COMPANY_LOGO_URL
                )
                if os.path.exists(logo_path):
                    with open(logo_path, "rb") as logo_file:
                        logo_data = logo_file.read()
            ...
            html_content = render_to_string(
                "loan-disbursed/loan_disbursed_fund_pdf.html", context
            )
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="loan_report_{loan_ref_no}.pdf"'
            )
            return response
    ...
    def _build_emi_payment_loan_list(self, branch):
        ...
        unpaid_refs = set(
            LoanEMISchedule.objects.filter(
                loan_application__in=applications, paid=False
            )
        ...
        approved_close_refs = set(
            LoanCloseRequest.objects.filter(
                loan_application__in=applications, status="approved"
            )
            .values_list("loan_application__loan_ref_no", flat=True)
            .distinct()
        )
        ...
        for app in applications:
            ...
            latest_reschedule_log = (
                LoanRescheduleLog.objects.filter(loan_application=app)
                .order_by("-created_at")
                .first()
            )
            if latest_reschedule_log is not None:
                res_qs = LoanEMIReschedule.objects.filter(
                    reschedule_log=latest_reschedule_log
                )
                if res_qs.exists():
            ...
            loan_detail = (
                app.loan_details.first() if hasattr(app, "loan_details") else None
            )
            loan_list.append(
                {
                    ...
                    "disbursed_date": (
                        app.disbursed_at.strftime("%Y-%m-%d")
                        if getattr(app, "disbursed_at", None)
                        else "N/A"
                    ),
                    "loan_amount": (
                        str(getattr(loan_detail, "loan_amount", "0.00"))
                        if loan_detail
                        else "0.00"
                    ),
...
class EmiPaymentRowsView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        html = render_to_string(
            self.template_name, {"loans": page_obj.object_list}, request=request
        )
        return HttpResponse(html)


@method_decorator(
    branch_permission_required("view_emis", "collect_emi", "receive_emi", "reject_emi"),
    name="dispatch",
)
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            permissions = {
                ...
                "can_collect_emi": employee.is_manager
                or employee.has_perm("collect_emi"),
                "can_receive_emi": employee.is_manager
                or employee.has_perm("receive_emi"),
                "can_reject_emi": employee.is_manager
                or employee.has_perm("reject_emi"),
            }
            ...
            if loan_ref_no:
                try:
                    ...
                    collected_rows = EmiCollectionDetail.objects.filter(
                        emi__in=emi_schedules, collected=True
                    ).values(
                        "emi_id",
                        "amount_received",
                        "collected_by_agent_id",
                        "collected_by_branch_id",
                    )
                    collected_map = {
                        str(r["emi_id"]): {
                    ...
                    for row in schedules_data:
                        ...
                        if coll and coll.get("amount_received") is not None:
                            ...
                            row["collected_by"] = (
                                "agent"
                                if coll.get("collected_by_agent_id")
                                else "branch"
                            )
                        else:
                    ...
                    all_paid = not LoanEMISchedule.objects.filter(
                    ...
                    latest_period = (
                        LoanPeriod.objects.filter(loan_application=loan)
                        .order_by("-created_at")
                        .first()
                    )
                    reschedule_count = (
                        latest_period.reschedule_count if latest_period else 0
                    )
                    ...
                    return Response(
                        {
                            ...
                            "customer_name": (
                                getattr(
                                    getattr(loan, "customer", None), "full_name", None
                                )
                                or ""
                            ),
                            ...
                            "last_installment_date": (
                                last_emi.installment_date if last_emi else None
                            ),
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not reschedule_emi_id or not loan_ref_no:
            return Response(
                {
                    "success": False,
                    "message": "reschedule_emi_id and loan_ref_no are required.",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )

        try:
            branch_manager = BranchEmployee.objects.select_related("branch").get(
                id=branch_manager_id
            )
        except BranchEmployee.DoesNotExist:
            return Response(
                {"success": False, "message": "Branch manager not found in session"},
                status=status.HTTP_403_FORBIDDEN,
            )

        # Resolve the rescheduled EMI and ensure it belongs to this branch and loan
        try:
            res_emi = LoanEMIReschedule.objects.select_related(
                "loan_application__branch"
            ).get(id=reschedule_emi_id, loan_application__loan_ref_no=loan_ref_no)
        except LoanEMIReschedule.DoesNotExist:
            return Response(
                {
                    "success": False,
                    "message": "Rescheduled EMI not found for this loan.",
                },
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        if getattr(res_emi.loan_application, "branch", None) != branch_manager.branch:
            return Response(
                {
                    "success": False,
                    "message": "Rescheduled EMI does not belong to your branch.",
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        try:
            with transaction.atomic():
                ...
                EmiCollectionDetail.objects.filter(reschedule_emi=res_emi).exclude(
            ...
            return Response(
                {
                    "success": False,
                    "message": f"Failed to save rescheduled EMI collection: {str(e)}",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class LoanRescheduleAPIView(APIView):
    ...
    def post(self, request, loan_ref_no, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {
                    "detail": "Unexpected error while rescheduling loan.",
                    "error": str(e),
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class LoanRescheduleEmiListAPIView(APIView):
    ...
    def get(self, request, loan_ref_no, *args, **kwargs):
        ...
        logs_qs = LoanRescheduleLog.objects.filter(loan_application=loan).order_by(
            "-created_at"
        )
        log = None
        ...
        for candidate in logs_qs:
            qs = LoanEMIReschedule.objects.filter(reschedule_log=candidate).order_by(
                "installment_date"
            )
            if qs.exists():
                log = candidate
        ...
        if log is None or not emis.exists():
            return Response(
                {"detail": "No reschedule data found for this loan."},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        history = []
        all_logs = LoanRescheduleLog.objects.filter(loan_application=loan).order_by(
            "created_at"
        )
        for hlog in all_logs:
            h_emis_qs = LoanEMIReschedule.objects.filter(reschedule_log=hlog).order_by(
                "installment_date"
            )
            ...
            for hemi in h_emis_qs:
                h_last_collection = (
                    EmiCollectionDetail.objects.filter(reschedule_emi=hemi)
                    .order_by("-collected_at")
                    .first()
                )
                h_collection_status = (
                    getattr(h_last_collection, "status", None)
                    if h_last_collection
                    else None
                )
                ...
                h_assigned_agent_name = (
                    ...
                    if h_active_assignment is not None
                    and getattr(h_active_assignment, "agent", None) is not None
                    else None
                )
            ...
            h_total_installment_amount = h_totals.get(
                "total_installment_amount"
            ) or Decimal("0.00")
            h_total_principal_amount = h_totals.get(
                "total_principal_amount"
            ) or Decimal("0.00")
            h_total_interest_amount = h_totals.get("total_interest_amount") or Decimal(
                "0.00"
            )
            ...
            h_remaining_balance = getattr(
                h_period, "remaining_balance", None
            ) or Decimal("0.00")
            h_remaining_principal = getattr(
                h_period, "remaining_principal", None
            ) or Decimal("0.00")
            h_remaining_interest = getattr(
                h_period, "remaining_interest", None
            ) or Decimal("0.00")
        ...
        for emi in emis:
            ...
            last_collection = (
                EmiCollectionDetail.objects.filter(reschedule_emi=emi)
                .order_by("-collected_at")
                .first()
            )
            collection_status = (
                getattr(last_collection, "status", None) if last_collection else None
            )
            ...
            assigned_agent_id = (
                ...
                and getattr(active_assignment, "agent", None) is not None
                else None
            )
            assigned_agent_name = (
        ...
        total_installment_amount = totals.get("total_installment_amount") or Decimal(
            "0.00"
        )
        ...
        remaining_balance = getattr(period, "remaining_balance", None) or Decimal(
            "0.00"
        )
        remaining_principal = getattr(period, "remaining_principal", None) or Decimal(
            "0.00"
        )
        remaining_interest = getattr(period, "remaining_interest", None) or Decimal(
            "0.00"
        )
...
class upcomingEMIView(TemplateView):
    ...
    def _build_upcoming_emi_data(self, branch):
        ...
        branch_agents = list(
            Agent.objects.filter(branch=branch, status="active")
            .order_by("full_name")
            ...
            emi_data.append(
                {
                    ...
                    "customer_name": (
                        emi.loan_application.customer.full_name
                        if emi.loan_application.customer
                        else "N/A"
                    ),
                    ...
                    "late_fee": (
                        float(emi.late_fee)
                        if hasattr(emi, "late_fee") and emi.late_fee is not None
                        else 0.0
                    ),
                    "paid_date": (
                        emi.paid_date.strftime("%Y-%m-%d") if emi.paid_date else None
                    ),
                }
            )
...
class upcomingEMIAPIView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            next_month_start = (today.replace(day=1) + timedelta(days=32)).replace(
                day=1
            )
...
class AssignAgentToEMI(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            try:
                emi_schedule = LoanEMISchedule.objects.select_related(
                    "loan_application"
                ).get(id=schedule_pk)
            ...
            if (
                getattr(emi_schedule.loan_application, "loan_ref_no", None)
                != loan_ref_no
            ):
                return Response(
            ...
            if getattr(emi_schedule, "paid", False):
                return Response(
                    {"detail": "Cannot assign agent to a paid EMI schedule."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            EmiAgentAssign.objects.filter(emi=emi_schedule, is_active=True).update(
                is_active=False
            )
            ...
            return Response(
                {"detail": "Branch manager not found"}, status=status.HTTP_404_NOT_FOUND
            )
        except LoanEMISchedule.DoesNotExist:
            return Response(
                {"detail": "EMI schedule not found"}, status=status.HTTP_404_NOT_FOUND
            )
...
class UnassignAgentFromEMI(APIView):
    def post(self, request, emi_id, *args, **kwargs):
        ...
        if not emi_id:
            return Response(
                {"detail": "emi_id is required."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if (
            getattr(emi_schedule.loan_application, "branch", None)
            != branch_manager.branch
        ):
            return Response(
                {"detail": "EMI schedule does not belong to your branch."},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        if getattr(emi_schedule, "paid", False):
            return Response(
                {"detail": "Cannot unassign agent from a paid EMI schedule."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        with transaction.atomic():
            ...
            if deleted_count == 0:
                return Response(
                    {"detail": "No assignment found for this EMI."},
                    status=status.HTTP_404_NOT_FOUND,
                )
        ...
        return Response(
            {
                "status": "success",
                "message": "Agent unassigned and assignments deleted successfully.",
            },
            status=status.HTTP_200_OK,
        )
...
class receiveEmiDetailAPI(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            emi = LoanEMISchedule.objects.select_related(
                "loan_application__branch"
            ).get(id=emi_id)
        except LoanEMISchedule.DoesNotExist:
            return Response(
                {"detail": "EMI not found."}, status=status.HTTP_404_NOT_FOUND
            )

        # Ensure EMI belongs to manager's branch
        if getattr(emi.loan_application, "branch", None) != branch_manager.branch:
            return Response(
                {"detail": "EMI does not belong to your branch."},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        collection = (
            EmiCollectionDetail.objects.filter(emi=emi)
            .order_by("-collected_at")
            .first()
        )
        if not collection:
            return Response(
                {"detail": "No collection record found for this EMI."},
                status=status.HTTP_404_NOT_FOUND,
            )

        period = (
            LoanPeriod.objects.filter(loan_application=emi.loan_application)
        ...
        try:
            ...
            return Response(
                {"detail": f"Invalid remaining values: {_remain_err}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
            ...
            return Response(
                {
                    ...
                    "remaining_balance": getattr(
                        emi.loan_application, "remaining_balance", None
                    ),
                    "remaining_principal": getattr(
                        emi.loan_application, "remaining_principal", None
                    ),
                    "remaining_interest": getattr(
                        emi.loan_application, "remaining_interest", None
                    ),
                },
                status=status.HTTP_200_OK,
            )
        ...
        try:
            with transaction.atomic():
                ...
                branch_account = BranchAccount.objects.select_for_update().get(
                    branch=branch_manager.branch, type="CASH"
                )
                if branch_account:
                    branch_account.current_balance += (
                        collection.amount_received + collection.penalty_received
                    )
                    branch_account.updated_at = timezone.now().date()
                    branch_account.save()

                total_amount = (collection.amount_received or Decimal("0")) + (
                    collection.penalty_received or Decimal("0")
                )
                ...
                try:
                    ...
                    next_emi_amount = (
                        getattr(next_emi, "installment_amount", None)
                        if next_emi
                        else None
                    )
                    next_emi_due_date = (
                        getattr(next_emi, "installment_date", None)
                        if next_emi
                        else None
                    )
                ...
                if next_emi:
                    unpaid_qs = LoanEMISchedule.objects.filter(
                        loan_application=emi.loan_application,
                        paid=False,
                        ...
                        unpaid_emis_to_next.append(
                            {
                                ...
                                "is_overdue": bool(
                                    s.installment_date and s.installment_date < today
                                ),
                            }
                        )
                    ...
                    if customer_email:
                        ...
                        try:
                            context = {
                                ...
                                or collection.payment_reference
                                or "N/A",
                                "next_emi_amount": (
                                    next_emi_amount
                                ...
                                "next_emi_due_date": (
                                    next_emi_due_date if next_emi_due_date else "N/A"
                                ),
                            ...
                            message_html = None
                            print(
                                f"[Email] Could not render EMI HTML template: {_tmpl_err}"
                            )
                        ...
                        print(
                            "[Email] Skipped sending EMI receipt: customer email not available"
                        )
                except Exception as e:
                ...
                return Response(
                    {
                        ...
                        "remaining_balance": (
                            getattr(period, "remaining_balance", None)
                            if period
                            else None
                        ),
                        "remaining_principal": (
                            getattr(period, "remaining_principal", None)
                            if period
                            else None
                        ),
                        "remaining_interest": (
                            getattr(period, "remaining_interest", None)
                            if period
                            else None
                        ),
                    },
            ...
            return Response(
                {"detail": f"Failed to verify collection: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class receiveRescheduleEmiDetailAPI(APIView):
    def post(self, request, *args, **kwargs):
        ...
        reschedule_emi_id = kwargs.get("reschedule_emi_id") or request.data.get(
            "reschedule_emi_id"
        )
        if not reschedule_emi_id:
            return Response(
                {"detail": "reschedule_emi_id is required."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        collection = (
            EmiCollectionDetail.objects.filter(reschedule_emi=res_emi)
            .order_by("-collected_at")
            .first()
        )
        if not collection:
            return Response(
                {"detail": "No collection record found for this rescheduled EMI."},
                status=status.HTTP_404_NOT_FOUND,
            )

        period = (
            LoanPeriod.objects.filter(loan_application=res_emi.loan_application)
            .order_by("-created_at", "-id")
            .first()
        )
        ...
        try:
            ...
            if period:
                ...
                sched_inst = getattr(
                    res_emi, "installment_amount", Decimal("0.00")
                ) or Decimal("0.00")
                sched_prin = getattr(
                    res_emi, "principal_amount", Decimal("0.00")
                ) or Decimal("0.00")
                sched_int = getattr(
                    res_emi, "interest_amount", Decimal("0.00")
                ) or Decimal("0.00")
                ...
                if remainder > 0:
                    ...
                    extra_prin = (remainder * prin_ratio).quantize(
                        Decimal("0.00"), rounding=ROUND_HALF_UP
                    )
                    extra_int = (remainder - extra_prin).quantize(
                        Decimal("0.00"), rounding=ROUND_HALF_UP
                    )
            ...
            return Response(
                {
                    ...
                    "status": collection.status,
                    "remaining_balance": (
                        getattr(period, "remaining_balance", None) if period else None
                    ),
                    "remaining_principal": (
                        getattr(period, "remaining_principal", None) if period else None
                    ),
                    "remaining_interest": (
                        getattr(period, "remaining_interest", None) if period else None
                    ),
                },
                status=status.HTTP_200_OK,
            )
        ...
        try:
            with transaction.atomic():
                ...
                def _mark_single_res_emi_paid(target_emi, apply_late_fee_and_ref=False):
                    ...
                    if apply_late_fee_and_ref:
                        ...
                        # Fill payment_reference from collection if available and not already set
                        if (
                            not target_emi.payment_reference
                            and collection.payment_reference
                        ):
                ...
                if extra_to_mark > 0:
                    ...
                    sibling_emis_qs = LoanEMIReschedule.objects.filter(
                        reschedule_log=res_emi.reschedule_log
                    ).order_by("installment_date", "id")
                    ...
                    try:
                        current_index = next(
                            idx
                            for idx, emi_obj in enumerate(sibling_emis)
                    ...
                    if current_index != -1:
                        ...
                        for emi_obj in sibling_emis[current_index + 1 :]:
                            ...
                            _mark_single_res_emi_paid(
                                emi_obj, apply_late_fee_and_ref=False
                            )
                            remaining_slots -= 1
            ...
            return Response(
                {"detail": f"Failed to verify rescheduled EMI collection: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class GetEmiScedulePaidDataAPI(APIView):
    def get(self, request):
        ...
        try:
            ...
            paid_emis = (
                LoanEMISchedule.objects.select_related("loan_application")
            ...
            return Response(
                {"detail": f"Failed to fetch paid EMI schedules: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class loanPaidEmiCollectedAPI(APIView):
    def get(self, request, loan_ref_no, *args, **kwargs):
        ...
        try:
            ...
            try:
                loan_app = LoanApplication.objects.get(
                    loan_ref_no=loan_ref_no, branch=branch
                )
            except LoanApplication.DoesNotExist:
                return Response(
                    {"detail": "Loan not found."}, status=status.HTTP_404_NOT_FOUND
...
class LoanRemainingAPI(APIView):
    def get(self, request, loan_ref_no, *args, **kwargs):
        ...
        try:
            loan_app = LoanApplication.objects.get(
                loan_ref_no=loan_ref_no, branch=manager.branch
            )
        except LoanApplication.DoesNotExist:
            return Response(
                {"detail": "Loan not found."}, status=status.HTTP_404_NOT_FOUND
        ...
        period = (
            LoanPeriod.objects.filter(loan_application=loan_app)
            .order_by("-created_at")
            .first()
        )
        if not period:
            return Response(
                {"detail": "Loan period not found for this loan."},
                status=status.HTTP_404_NOT_FOUND,
            )
...
class DueEmiView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for emi in combined_emis:
            ...
            date_str = (
                installment_date.strftime("%Y-%m-%d") if installment_date else None
            )
            time_str = "00:00"
            days_remaining = (installment_date - today).days if installment_date else 0
            status_val = (
                "Upcoming" if installment_date and installment_date > today else "Due"
            )
            ...
            emi_data.append(
                {
                    ...
                    "loan_ref_no": getattr(
                        getattr(emi, "loan_application", None), "loan_ref_no", None
                    ),
                    "customer_name": (
                        getattr(
                            getattr(
                                getattr(emi, "loan_application", None), "customer", None
                            ),
                            "full_name",
                            None,
                        )
                    ...
                    "assigned_agent_name": assigned_agent_name,
                    "late_fee": (
                        float(getattr(emi, "late_fee", 0) or 0)
                        if hasattr(emi, "late_fee")
                        else 0.0
                    ),
                    "paid_date": getattr(emi, "paid_date", None),
                }
        ...
        try:
            context["due_page_links"] = list(
                paginator.get_elided_page_range(
                    due_emis_page.number, on_each_side=2, on_ends=1
                )
            )
        except Exception:
    ...
    def get(self, request):
        ...
        try:
            ...
            if emi_data:
                ...
                return Response(
                    {"detail": "No due EMIs found."}, status=status.HTTP_404_NOT_FOUND
                )
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            context["overdue_page_links"] = list(
                paginator.get_elided_page_range(
                    overdue_emis_page.number, on_each_side=2, on_ends=1
                )
            )
        except Exception:
...
class LoanCloseRequestAPI(APIView):
    def get(self, request, loan_ref_no=None, *args, **kwargs):
        ...
        if not loan_ref_no:
            return Response(
                {"detail": "loan_ref_no is required."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"detail": "Loan not found for this branch."},
                status=status.HTTP_404_NOT_FOUND,
            )

        # Look for a pending request
        existing = LoanCloseRequest.objects.filter(
            loan_application=loan, status="pending"
        ).first()
        ...
        last_req = (
            LoanCloseRequest.objects.filter(loan_application=loan)
            .order_by("-requested_at")
            .first()
        )
...
class BranchDashboardStatsAPI(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if req_branch_id and str(req_branch_id) != str(branch.branch_id):
            return Response(
                {"detail": "Forbidden: branch mismatch."},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if time_range == "day":
            ...
            if month < 1 or month > 12:
                return Response(
                    {"detail": "Invalid month."}, status=status.HTTP_400_BAD_REQUEST
                )
            dim = _calendar.monthrange(year, month)[1]
            if day < 1 or day > dim:
                return Response(
                    {"detail": "Invalid day for the selected month."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        savings_stats = {
            ...
            "pending": savings_base_qs.filter(
                status__in=savings_pending_statuses
            ).count(),
            "approved": savings_base_qs.filter(
                status__in=savings_approved_statuses
            ).count(),
            "disbursed": savings_base_qs.filter(
                status__in=savings_disbursed_statuses
            ).count(),
            "rejected": savings_base_qs.filter(
        ...
        if time_range == "year":
            ...
            for y in years:
                ...
                count = LoanApplication.objects.filter(
                    branch=branch, submitted_at__year=y
                ).count()
                trends.append({"label": str(y), "count": count})
                emi_categories.append(str(y))
                emi_scheduled.append(
                    LoanEMISchedule.objects.filter(
                        loan_application__branch=branch, installment_date__year=y
                    ).aggregate(total=Sum("installment_amount"))["total"]
                    or 0
                )
                emi_collected.append(
                    ...
                    .filter(
                        Q(collected_by_agent__isnull=False)
                        | Q(collected_by_branch__isnull=False)
                    )
            ...
            if month == 12:
                ...
                month_end = timezone.make_aware(
                    datetime(year, month + 1, 1, 0, 0, 0), tz
                )
            ...
            month_qs = (
                ...
                .annotate(d=TruncDay("submitted_at"))
                .values("d")
                .annotate(c=Count("pk"))
                .order_by("d")
            )
            ...
            for d in range(1, days_in_month + 1):
                ...
                emi_scheduled.append(
                    LoanEMISchedule.objects.filter(
                        loan_application__branch=branch, installment_date=dt
                    ).aggregate(total=Sum("installment_amount"))["total"]
                    or 0
                )
            ...
            for yy, mm in months:
                ...
                c = LoanApplication.objects.filter(
                    branch=branch, submitted_at__gte=start_m, submitted_at__lt=next_m
                ).count()
        ...
        prev_approved = LoanApplication.objects.filter(
            branch=branch,
            approved_at__isnull=False,
            approved_at__gte=prev_start,
            approved_at__lt=prev_end,
        ).count()
        ...
        recent_loans_qs = (
            LoanApplication.objects.filter(branch=branch)
        ...
        for la in recent_loans_qs:
            ...
            first_detail = (
                la.loan_details.first() if hasattr(la, "loan_details") else None
            )
            amount_val = (
                float(first_detail.loan_amount)
                if first_detail and first_detail.loan_amount is not None
                else 0.0
            )
            recent_loans.append(
                {
                    ...
                    "amount": amount_val,
                    "date": (
                        la.submitted_at.strftime("%Y-%m-%d") if la.submitted_at else ""
                    ),
                    "status": la.status,
                }
            )
        ...
        agent_rows = (
            LoanApplication.objects.filter(branch=branch, agent__isnull=False)
        ...
        if not recent_agents:
            ...
            fallback_agents = (
                Agent.objects.filter(branch=branch)
                .values("agent_id", "full_name")
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            agg = qs.aggregate(
                ...
                penalty_total=Sum("penalty_received"),
            )
            ...
            for coll in qs.select_related("emi", "reschedule_emi"):
                emi_obj = getattr(coll, "reschedule_emi", None) or getattr(
                    coll, "emi", None
                )
                freq = (
                    (getattr(emi_obj, "frequency", None) or "").lower()
                    if emi_obj
                    else ""
                )
...
class AgentDepositReceiveAPIView(APIView):
    def post(self, request, agent_id):
        try:
            ...
            online_amount = Decimal(str(data.get("online_amount") or 0)).quantize(
                Decimal("0.00")
            )
            ...
            agg = EmiCollectionDetail.objects.filter(
                collected_by_agent__agent_id=agent_id, status="collected"
            ).aggregate(
                amount_total=Sum("amount_received"),
                penalty_total=Sum("penalty_received"),
            )
            server_expected = (agg.get("amount_total") or Decimal("0")) + (
                agg.get("penalty_total") or Decimal("0")
            )
            emi_expected_total = (
                server_expected
                if isinstance(server_expected, Decimal)
                else Decimal(str(server_expected))
            ).quantize(Decimal("0.00"))

            savings_agg = SavingsCollection.objects.filter(
                collected_by_agent__agent_id=agent_id,
                is_collected=True,
                is_deposited_to_branch=False,
            ...
            expected_total = (emi_expected_total + savings_expected_total).quantize(
                Decimal("0.00")
            )
            ...
            if expected_total == 0 and online_amount > 0:
                return Response(
                    {"detail": "Amount mismatch"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            received_by = None
            if hasattr(request.user, "branchemployee") and getattr(
                request.user, "branchemployee", None
            ):
                ...
                if received_by is None:
                    ...
                    if not received_by_id:
                        return Response(
                            {"detail": "received_by is required"}, status=400
                        )
                    try:
                        received_by = BranchEmployee.objects.get(
                            employee_id=received_by_id
                        )
                    except BranchEmployee.DoesNotExist:
            ...
            if not received_by.branch:
                return Response(
                    {"detail": "Receiver is not assigned to a branch"}, status=400
                )
            ...
            if online_amount > 0:
                try:
                    selected_account = BranchAccount.objects.get(
                        id=branch_account_id, branch=branch, type="BANK"
                    )
                except BranchAccount.DoesNotExist:
                    return Response(
            ...
            for d in denominations:
                ...
                line_total = (Decimal(value) * Decimal(max(qty, 0))).quantize(
                    Decimal("0.00")
                )
                if coin < 0:
                    coin = 0
            ...
            if grand_total > expected_total:
                return Response(
                    {"detail": "Grand Total amount is greater than collected amount"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            # 2) Optionally reject when collected exceeds entered Grand Total (kept for safety)
            if expected_total > grand_total:
                return Response(
                    {"detail": "Collected amount exceeds Grand Total entered"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            base_daily = Decimal(str(categories.get("daily") or 0)).quantize(
                Decimal("0.00")
            )
            base_weekly = Decimal(str(categories.get("weekly") or 0)).quantize(
                Decimal("0.00")
            )
            base_saving = Decimal(str(categories.get("saving") or 0)).quantize(
                Decimal("0.00")
            )
            base_others = Decimal(str(categories.get("others") or 0)).quantize(
                Decimal("0.00")
            )
            ...
            coll_for_penalty = EmiCollectionDetail.objects.select_related(
                "emi", "reschedule_emi"
            ...
            for coll in coll_for_penalty:
                ...
                if not pen:
                    continue
                emi_obj = getattr(coll, "emi", None) or getattr(
                    coll, "reschedule_emi", None
                )
                freq = (
                    getattr(emi_obj, "frequency", None) if emi_obj is not None else None
                )
                if freq == "daily":
                    daily_penalty += pen
                elif freq == "weekly":
            ...
            with transaction.atomic():
                ...
                den_objs = [
                    AgentDepositDenomination(
                        deposit=dep, value=v, qty=q, line_total=lt, coin=c, cash=cs
                    )
                ...
                try:
                    ...
                    _logger.error(
                        f"Error updating BranchAccount balances for deposit {dep.deposit_id}: {_acct_err}"
                    )

                # After successful deposit, mark the agent's collected EMIs as paid (all-time, no date filter)
                try:
                    today = timezone.localdate()
                    collections_qs = EmiCollectionDetail.objects.select_related(
                        "emi", "reschedule_emi", "loan_application"
                    ).filter(collected_by_agent=agent, status__in=["collected"])
                    for coll in collections_qs:
                        ...
                        if emi_obj:
                            ...
                            if not getattr(emi_obj, "payment_reference", None):
                                emi_obj.payment_reference = (
                                    getattr(coll, "collected_id", None)
                                    or f"DEP-{dep.deposit_id}"
                                )
                            emi_obj.late_fee = getattr(
                                coll, "penalty_received", Decimal("0.00")
                            ) or Decimal("0.00")
                            emi_obj.save(
                                update_fields=[
                                    "paid",
                    ...
                    for coll in collections_qs:
                        ...
                        g["total_principal"] += coll.principal_received or Decimal(
                            "0.00"
                        )
                        ...
                        if emi_obj:
                            g[
                                "scheduled_amount"
                            ] += emi_obj.installment_amount or Decimal("0.00")
                            g[
                                "scheduled_principal"
                            ] += emi_obj.principal_amount or Decimal("0.00")
                            g[
                                "scheduled_interest"
                            ] += emi_obj.interest_amount or Decimal("0.00")
                            if (
                                g["max_emi_date"] is None
                                or emi_obj.installment_date > g["max_emi_date"]
                            ):
                    ...
                    for loan_ref, aggvals in loan_groups.items():
                        ...
                        rem_balance = (
                            (period.remaining_balance or Decimal("0.00"))
                            if period
                            else Decimal("0.00")
                        )
                        rem_principal = (
                            (period.remaining_principal or Decimal("0.00"))
                            if period
                            else Decimal("0.00")
                        )
                        rem_interest = (
                            (period.remaining_interest or Decimal("0.00"))
                            if period
                            else Decimal("0.00")
                        )
                        ...
                        extra_amount = (
                            aggvals["total_amount"]
                            + rem_balance
                            - aggvals["scheduled_amount"]
                        )
                        extra_principal = (
                            aggvals["total_principal"]
                            + rem_principal
                            - aggvals["scheduled_principal"]
                        )
                        extra_interest = (
                            aggvals["total_interest"]
                            + rem_interest
                            - aggvals["scheduled_interest"]
                        )
                        if extra_amount < 0:
                        ...
                        if remainingForFulls <= 0:
                            ...
                            if period:
                                ...
                                if (
                                    new_rb == Decimal("0.00")
                                    and aggvals.get("max_emi_date") is not None
                                ):
                                    ...
                                    for emi_row in emis_to_check:
                                        sums = EmiCollectionDetail.objects.filter(
                                            emi=emi_row,
                                            status__in=["collected", "verified"],
                                        ).aggregate(
                                            amt=Sum("amount_received"),
                                            pen=Sum("penalty_received"),
                                        )
                                        total_paid = (
                                            sums.get("amt") or Decimal("0.00")
                                        ) + (sums.get("pen") or Decimal("0.00"))
                                        due_amount = (
                                            emi_row.installment_amount
                                            or Decimal("0.00")
                                        ) + (emi_row.late_fee or Decimal("0.00"))
                                        if (
                                            total_paid >= due_amount
                                            and not emi_row.paid
                                        ):
                                            emi_row.paid = True
                                            if not emi_row.paid_date:
                                                emi_row.paid_date = today
                                            emi_row.save(
                                                update_fields=["paid", "paid_date"]
                                            )

                                has_unpaid = LoanEMISchedule.objects.filter(
                                    loan_application=loan_app, paid=False
                                ).exists()
                                if not has_unpaid:
                                    new_rb = Decimal("0.00")
                                    new_rp = Decimal("0.00")
                        ...
                        using_reschedule_plan = LoanEMIReschedule.objects.filter(
                            loan_application=loan_app
                        ).exists()
                        ...
                        for row in emi_qs:
                            ...
                            if using_reschedule_plan:
                                ...
                                if (
                                    remainingForFulls < rowEmi
                                    or remainingPrincipal < rowPrincipal
                                    or remainingInterest < rowInterest
                                ):
                                    break
                            ...
                            if using_reschedule_plan:
                                assign_qs = EmiAgentAssign.objects.filter(
                                    reschedule_emi=row, is_active=True
                                )
                            else:
                                assign_qs = EmiAgentAssign.objects.filter(
                                    emi=row, is_active=True
                                )

                            try:
                                assign_obj = (
                                    assign_qs.filter(agent=agent)
                                    .order_by("-assigned_at")
                                    .first()
                                    or assign_qs.order_by("-assigned_at").first()
                                )
                            except Exception:
                                assign_obj = None
                            ...
                            new_coll = EmiCollectionDetail.objects.create(
                                **new_coll_kwargs
                            )
                            ...
                            row.save(
                                update_fields=["paid", "paid_date", "payment_reference"]
                            )

                            # Deduct budgets
                            remainingForFulls = (remainingForFulls - rowEmi).quantize(
                                Decimal("0.00")
                            )
                            remainingPrincipal = (
                                remainingPrincipal - rowPrincipal
                            ).quantize(Decimal("0.00"))
                            remainingInterest = (
                                remainingInterest - rowInterest
                            ).quantize(Decimal("0.00"))
                        ...
                        if (
                            ...
                            for emi_row in emis_to_check:
                                sums = EmiCollectionDetail.objects.filter(
                                    emi=emi_row, status__in=["collected", "verified"]
                                ).aggregate(
                                    amt=Sum("amount_received"),
                                    pen=Sum("penalty_received"),
                                )
                                already_paid = (sums.get("amt") or Decimal("0.00")) + (
                                    sums.get("pen") or Decimal("0.00")
                                )
                                due_amount = (
                                    emi_row.installment_amount or Decimal("0.00")
                                ) + (emi_row.late_fee or Decimal("0.00"))

                                if emi_row.paid:
                                    continue

                                needed_from_pool = (due_amount - already_paid).quantize(
                                    Decimal("0.00")
                                )
                                ...
                                if pool >= needed_from_pool:
                                    pool = (pool - needed_from_pool).quantize(
                                        Decimal("0.00")
                                    )
                                    emi_row.paid = True
                                    if not emi_row.paid_date:
                        ...
                        if period:
                            ...
                            if using_reschedule_plan:
                                has_unpaid = LoanEMIReschedule.objects.filter(
                                    loan_application=loan_app, paid=False
                                ).exists()
                            else:
                                has_unpaid = LoanEMISchedule.objects.filter(
                            ...
                            if not has_unpaid:
                                ...
                                if not using_reschedule_plan:
                                    rb_pool = period.remaining_balance or Decimal(
                                        "0.00"
                                    )
                                    if rb_pool > 0:
                                        unpaid_qs = LoanEMISchedule.objects.filter(
                                        ...
                                        for emi_row in unpaid_qs:
                                            ...
                                            if rb_pool >= emi_due and emi_due > 0:
                                                rb_pool = (rb_pool - emi_due).quantize(
                                                    Decimal("0.00")
                                                )
                                                emi_row.paid = True
                                                if not emi_row.paid_date:
                ...
                if payment_mode == "Mixed":
                    ...
                    if online_amount and online_amount > 0:
                        if selected_account is None:
                            raise ValueError(
                                "Selected bank account not found for online deposit"
                            )
                        BranchTransaction.objects.create(
                            branch=branch,
                    ...
                    if dep.daily_amount and dep.daily_amount > 0:
                        ...
                        BranchTransaction.objects.create(
                            ...
                            transaction_type="CREDIT",
                            purpose=(
                                coa_daily.head_of_account if coa_daily else "Daily EMI"
                            ),
...
class AgentDepositPreviousAPIView(APIView):
    def get(self, request, agent_id):
        try:
            ...
            deposits = (
                AgentDeposit.objects.filter(agent=agent)
                .select_related("branch", "received_by")
            ...
            for deposit in deposits:
                deposit_data = {
                    ...
                    "received_by_name": (
                        deposit.received_by.get_full_name()
            ...
            return Response(
                {"success": False, "detail": "Agent not found"},
                status=status.HTTP_404_NOT_FOUND,
            )
        except Exception as e:
            return Response(
...
class AddAccount(APIView):
    def post(self, request):
        ...
        try:
            ...
            if BranchAccount.objects.filter(
                account_number=account_number, branch=branch
            ).exists():
                errors["account_number"] = (
                    "Account number already exists for this branch."
                )
